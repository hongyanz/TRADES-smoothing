...
def trades_loss(model,
    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                loss_kl = criterion_kl(F.log_softmax(model(x_adv+noise), dim=1),
        ...
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                loss = (-1) * criterion_kl(F.log_softmax(model(adv+noise), dim=1),
                                           F.softmax(model(x_natural+noise), dim=1))
            loss.backward()
            # renorming gradient
    ...
    loss_robust = (1.0 / batch_size) * criterion_kl(F.log_softmax(model(x_adv+noise), dim=1),
                                                    F.softmax(model(x_natural+noise), dim=1))
